...
class individual_graph(basic_model):
    def __init__(self, Size_field, pos, type_person=1):
        self.state = 0  # 0 for dead, 1 for susceptible, 2 for sick, 3 for immune
        self.size_field = Size_field
        ...
        person_position = sg.SceneGraphNode("person_position")
        person_position.transform = tr.translate(self.field_to_graph_coordinates(pos[0]),
                                                 self.field_to_graph_coordinates(pos[1]), 0)
        person_position.childs += [person_scaled]

        person = sg.SceneGraphNode("person")
        person.transform = tr.uniformScale(0.93)

        person.childs += [person_position]
        self.model = person

    def get_gpu_color_quad(self, state):
        """
        Get gpu representation of model, for their diferent states.
        0 for dead, 1 for susceptible, 2 for sick, 3 for immune
        :param state:
        :return:
        """
        assert (type(state) == int), ("Input should be integer")

        if state == 3:
            return es.toGPUShape(bs.createColorQuad(0.5, 0.5, 0.5))  # grey
        elif state == 1:
            return es.toGPUShape(bs.createColorQuad(0, 0, 1))  # blue
        elif state == 2:
            return es.toGPUShape(bs.createColorQuad(1, 0, 0))  # red
        elif state == 0:
            return es.toGPUShape(bs.createColorQuad(0.0, 0.0, 0))  # death
        else:
            raise Exception('Error. input should not exceed 3. The value of x was: {:d}'.format(state))

    def field_to_graph_coordinates(self, pos):
        # print("pos: {:.2f}".format((pos[0] - self.size_field / 2) / (self.size_field / 2)))
        return (pos - self.size_field / 2) / (self.size_field / 2)

    def get_coordinates(self):
        return self.field_to_graph_coordinates(self.position_x), self.field_to_graph_coordinates(self.position_y)

    def update_location(self, location):
        self.model.childs[0].transform = tr.translate(self.field_to_graph_coordinates(location[0]),
                                                      self.field_to_graph_coordinates(location[1]), 0)

    def update_state(self, new_state):
        self.square.childs = [self.get_gpu_color_quad(new_state)]


class center_graph(basic_model):
    def __init__(self, Size_field, pos):
        self.state = 0  # 0 for dead, 1 for susceptible, 2 for sick, 3 for immune
        self.size_field = Size_field
        self.position_y = pos[1]
        self.position_x = pos[0]
        ...
        center_rotated = sg.SceneGraphNode("center_rotated")
        center_rotated.transform = tr.rotationZ(-3.1415 / 4)
        center_rotated.childs += [center_scaled]

        # self.square = person_scaled

        center_position = sg.SceneGraphNode("center_position")
        center_position.transform = tr.translate(self.field_to_graph_coordinates(pos[0]),
                                                 self.field_to_graph_coordinates(pos[1]), 0)
        center_position.childs += [center_rotated]
        # center_position.childs += [center_scaled]

        center = sg.SceneGraphNode("center")
        center.transform = tr.uniformScale(0.93)

        center.childs += [center_position]
        self.model = center
